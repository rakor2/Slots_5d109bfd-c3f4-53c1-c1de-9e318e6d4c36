...
local _DebugMetaTable = {
    __index = function(t, k)
        Ext.Utils.Print(string.format("Accessing key '%s' from table-class: '%s' '%s'", k, t[_Index], _Class:GetClass(t[_Index]) ~= nil and _Class:GetClass(t[_Index])._ClassName or "N/A"))
        return t[_Index][k]
    end,
    __newindex = function(t, k, v)
        Ext.Utils.Print(string.format("Writing key-value '%s' '%s' to table-class: '%s' '%s'", k, v, t[_Index], _Class:GetClass(t[_Index]) ~= nil and _Class:GetClass(t[_Index])._ClassName or "N/A"))
        t[_Index][k] = v
    end
}
