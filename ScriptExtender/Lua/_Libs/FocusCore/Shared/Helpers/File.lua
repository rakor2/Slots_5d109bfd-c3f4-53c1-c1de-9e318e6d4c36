...
function Helpers.File:LoadStats(modDirectoryName, files, debug)
    ...
    if debug then
        FCDebug("Finished loading stat files.")
    end
end
...
function Helpers.File:LoadLoca(files, debug)
    ...
    if debug then
        FCDebug("Finished loading loca files.")
    end
end
...
function Helpers.File:DumpToFile(object, fileName)
    ...
    if contents ~= nil then
        ...
        FCDebug("Created file %s", fileName)
    else
        FCDebug("Could not contruct contents %s to save to file %s", object, fileName)
    end
end
