...
if Helpers.Commander.IsServer then
    Ext.Events.SessionLoaded:Subscribe(function()
        local function restore()
            ...
            Helpers.Timer:LaunchRealtimeObjectTimerOneShot(_C(), 200, "FocusCoreRestore", function()
                --Apply statuses here
            end)
        end
        Ext.Events.ResetCompleted:Subscribe(restore)
        Helpers.Commander:Register("Restore", function() restore() Osi.RestoreParty(Osi.GetHostCharacter()) end)
        Helpers.Commander:Register("RestoreResources", restore)
        Helpers.Commander:Register("ApplyStatus", function(status) Osi.ApplyStatus(Osi.GetHostCharacter(), status, -1, 1, Osi.GetHostCharacter()) end)
        Helpers.Commander:Register("GenTT", function(tt) Helpers.Inventory:GenerateTreasureTable(tt) end)
    end)
end

Ext.Events.SessionLoaded:Subscribe(function()
    Helpers.Commander:Register("DumpFile", function(guid) guid = guid or Helpers.Object:GetHostEntity().Uuid.EntityUuid; Ext.IO.SaveFile("Dump.json", Ext.DumpExport(Ext.Entity.Get(guid):GetAllComponents())) end)
end)